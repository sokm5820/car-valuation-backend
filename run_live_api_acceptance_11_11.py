#!/usr/bin/env python3
"""Live API structural + quality acceptance for OtoDeğer AI 11.11 Gold RC.
...
EXPECTED_BUILD = "11.11-gold-release-candidate"
...
def write_report(path: Path, base: str, results: List[dict]) -> None:
    lines = ["# OtoDeğer 11.11 — Live API Acceptance", "", f"Backend: `{base}`", f"Expected build: `{EXPECTED_BUILD}`", ""]
    for r in results:
