#!/usr/bin/env python3
"""Live API structural + quality acceptance for OtoDeğer AI 11.8 Gold RC.
...
def write_report(path: Path, base: str, results: List[dict]) -> None:
    lines = ["# OtoDeğer 11.8 — Live API Acceptance", "", f"Backend: `{base}`", f"Expected build: `{EXPECTED_BUILD}`", ""]
    for r in results:
...
def main() -> int:
    ...
    ap.add_argument("--output-prefix", default="live_api_acceptance_11_8")
