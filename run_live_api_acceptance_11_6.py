#!/usr/bin/env python3
"""Live API structural acceptance for OtoDeğer AI 11.6 Gold RC.
...
def case_p4(base: str) -> List[Session]:
    s = Session(base)
    action_is(s.send("I have £15,000. I want a small family car that's good on fuel. What should I look at?"), "SEARCH_VEHICLES")
    common(s.send("I like the Suzuki Swift."))
...
def case_p7(base: str) -> List[Session]:
    s = Session(base)
    action_is(s.send("Is £5,000 a good price for a 2007 Nissan March 1.2L?"), "EVALUATE_PURCHASE")
...
def write_report(path: Path, base: str, results: List[dict]) -> None:
    lines = ["# OtoDeğer 11.6 — Live API Acceptance", "", f"Backend: `{base}`", f"Expected build: `{EXPECTED_BUILD}`", ""]
    for r in results:
...
def main() -> int:
    ...
    ap.add_argument("--output-prefix", default="live_api_acceptance_11_6")
