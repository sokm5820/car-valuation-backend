...
- Aim for 45-75 words per recommendation: usually 2-3 concise sentences with a distinctive upside, a specific comparison with another shortlisted choice, and a material caveat or alternative use-case. A separate evidence card row displays all five raw metrics, so do NOT recite them field-by-field; interpret only the figures that genuinely change the stocking decision.
...
- Focus first on the commercial story: what these numbers MEAN for a stock decision, how this option compares with the other shortlisted vehicles, and what kind of stocking strategy it best suits. The evidence strip directly below already shows active listings, historical depth, 60-day exit share, median observed exit and median asking price.
- Treat the prose and evidence strip as complementary layers. The strip owns the raw facts; the prose owns the interpretation. Do not walk through the strip field-by-field and do not repeat all of its figures in sentence form.
- Use specific numbers SELECTIVELY, with context; do not impose an arbitrary ban on numbers. A statistic earns a place in the prose when it explains a meaningful contrast or decision: e.g. "65% of eligible historical listings were no longer advertised within 60 days, 12 percentage points higher than the next option—making this the strongest observed 60-day exit signal in the shortlist." If the difference is small or peers lack compatible evidence, do not claim a significant lead. Usually one standout comparative figure (occasionally two) is more useful than reciting all five tiles. You may instead compare median observed exit days, current supply, historical depth, or historical price-reduction pressure when THAT distinction is more commercially significant.
- Compare the candidate with the other supplied candidates, not an invented market average. When contrasting two options, identify the other vehicle by its name or a clear reference such as "the Honda above"; never say "best", "fastest", "highest", "lighter than most" or "by far" without a supported difference. A gap of a single day, one percentage point or one listing should not be exaggerated.
- Turn the comparison into a practical stocking implication: quicker observed stock rotation, a less crowded retail market, greater confidence in the demand signal, or greater sensitivity to the initial retail price. Then mention one genuine weakness where supported. Avoid filler such as "credible candidate", "strong overall balance" or "distinct role" with no concrete reason.
- Every recommendation should answer: (1) what is specifically different about this option versus the shortlist, (2) why that difference matters for the dealer's stock-turnover decision, and (3) what would make one of the other options preferable. Don't force a downside if the evidence does not support one. State missing or thin evidence naturally, without falsely implying certainty.
- Focus first on the commercial story: turnover strength, current supply pressure, and the most relevant pricing signal. Mention only the facts that explain WHY this option belongs where it does. Add a supported downside or condition only if it genuinely changes a stock decision, and otherwise explain which sourcing strategy the option suits.
...
- A strong example of the intended writing (ONLY when the actual candidate values support it): "Its 65% observed 60-day exit share is 12 points ahead of the next shortlisted choice, while current supply remains modest. That combination gives this variant the clearest quick-turn case here; the drawback is a much smaller eligible history than the Honda, so the apparent lead carries more uncertainty." These are illustrative figures, not facts about the user's vehicles. Use the REAL comparison gaps in the supplied data, or omit the example entirely when unsupported.
