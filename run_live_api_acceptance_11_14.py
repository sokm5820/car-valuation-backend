#!/usr/bin/env python3
"""Live API structural + quality acceptance for OtoDeğer AI 11.14 Gold RC.
...
EXPECTED_BUILD = "11.14-gold-release-candidate"
...
def case_p7(base: str) -> List[Session]:
    ...
    common(first)
    require(first.get("action") == "EVALUATE_PURCHASE", f"P7 turn 1 misrouted: expected EVALUATE_PURCHASE, got {first.get('action')!r}")
    ...
    common(r)
    require(r.get("action") == "EVALUATE_PURCHASE", f"P7 turn 2 variant refinement misrouted: expected EVALUATE_PURCHASE, got {r.get('action')!r}")
...
def case_b6(base: str) -> List[Session]:
    ...
    action_is(r, "ANALYZE_BUSINESS_PERIOD")
    ans=str(r.get("answer") or "").casefold()
    require("by price band" in ans or "price band" in ans, "B6 follow-up did not return a price-band breakdown")
    require("by fuel" in ans or "fuel" in ans, "B6 follow-up did not return a fuel breakdown")
    require("no verified price-band" not in ans and "cannot reliably rank" not in ans, "B6 falsely claimed requested segment evidence was unavailable")
    require("not confirmed sales" in ans or "not confirmed sale" in ans, "B6 did not preserve listing-exit vs confirmed-sale distinction")
    return [s]
...
def write_report(path: Path, base: str, results: List[dict]) -> None:
    lines = ["# OtoDeğer 11.14 — Live API Acceptance", "", f"Backend: `{base}`", f"Expected build: `{EXPECTED_BUILD}`", ""]
    for r in results:
...
def main() -> int:
    ...
    ap.add_argument("--output-prefix", default="live_api_acceptance_11_14")
